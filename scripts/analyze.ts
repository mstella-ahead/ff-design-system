...
/**
 * Emit `tokens/tokens.css` — the token set as CSS custom properties.
 *
 * This is what makes the system consumable: drop one stylesheet into a prototype
 * and the whole palette, scale and geometry is available. Fully generated, so it
 * is regenerated on every `npm run analyze` and must never be hand-edited — the
 * hand-written composition layer lives in `tokens/utilities.css` instead.
 *
 * Everything is prefixed `--ff-` rather than reusing the theme's bare `--primary`
 * etc. Bare names are exactly what collided with BB PowerPack's `--color-primary`
 * on the real site; a prototype that also loads Bootstrap or a page builder would
 * hit the same problem.
 */
function buildTokensCss(args: {
  clusters: ColorCluster[];
  scale: { steps: ScaleStep[]; ratio: number | null };
  typo: { baseFamily: string; baseSize: number };
  typoData: TypographyData;
  radiusUniform: Array<[string, number]>;
  shadow: Array<[string, number]>;
  respVars: Array<{ name: string; byWidth: Map<number, string> }>;
}): string {
  const { clusters, scale, typo, typoData, radiusUniform, shadow } = args;
  const L: string[] = [];

  L.push('/*');
  L.push(' * FormFactor design tokens — CSS custom properties.');
  L.push(' *');
  L.push(' * GENERATED by scripts/analyze.ts. Do not hand-edit; your changes will be');
  L.push(' * overwritten on the next `npm run analyze`. For the composition layer');
  L.push(' * (.ff-flow, .ff-radius, buttons, …) see tokens/utilities.css.');
  L.push(' *');
  L.push(' * Names follow FormFactor\'s own :root vocabulary, prefixed --ff- to avoid the');
  L.push(' * collision that bit the real site: BB PowerPack also defines --color-primary,');
  L.push(' * and it is #4a8eff, not FormFactor\'s navy.');
  L.push(' */');
  L.push('');
  L.push(':root {');

  // Colors — authored names first, then the un-authored observed ones.
  L.push('  /* ---- Color: brand + theme (FormFactor\'s own names) ---- */');
  const authoredClusters = clusters.filter((c) => c.authored && !c.social);
  for (const cl of authoredClusters) {
    const name = cl.authored ?? cl.generated;
    const role = cl.roles.text >= cl.roles.bg && cl.roles.text >= cl.roles.border ? 'text'
      : cl.roles.bg >= cl.roles.border ? 'background' : 'border';
    const note = cl.pseudoOnly ? ', via ::before/::after only' : '';
    const pad = ' '.repeat(Math.max(1, 22 - name.length - cl.rep.hex.length));
    L.push(`  --ff-${name}: ${cl.rep.hex};${pad}/* ${cl.count}x, ${cl.pages.size} pages, mostly ${role}${note} */`);
  }
  const unauthored = clusters.filter((c) => !c.authored && !c.social);
  if (unauthored.length) {
    L.push('');
    L.push('  /* ---- Color: observed but unnamed by the theme (generic UI greys, plugin');
    L.push('     defaults). Present for fidelity; prefer the named tokens above. ---- */');
    for (const cl of unauthored) L.push(`  --ff-${cl.generated}: ${cl.rep.hex};`);
  }
  const social = clusters.filter((c) => c.social);
  if (social.length) {
    L.push('');
    L.push('  /* ---- Color: social platforms. NOT FormFactor colors — only for share rows. ---- */');
    const label = (hex: string): string => ({ '#1e75b4': 'linkedin', '#2bacdb': 'x-twitter', '#2d3a8b': 'facebook' }[hex] ?? 'social');
    for (const cl of social) L.push(`  --ff-social-${label(cl.rep.hex)}: ${cl.rep.hex};`);
  }

  // Spacing — the authored modular scale.
  L.push('');
  L.push(`  /* ---- Spacing: the authored --size-* modular scale${scale.ratio ? `, ratio ${scale.ratio}` : ''} ---- */`);
  L.push('  --ff-base-size: 1;');
  for (const st of scale.steps) {
    const name = st.name.replace(/^size-/, '');
    L.push(`  --ff-space-${name}: ${st.px}px;${' '.repeat(Math.max(1, 8 - name.length))}/* ${st.multiplier}x base */`);
  }
  L.push('  --ff-flow-space: var(--ff-space-600);  /* default vertical rhythm */');

  // Typography
  L.push('');
  L.push('  /* ---- Typography ---- */');
  // The site's computed font-family is bare `"Proxima Nova"` with no fallback —
  // it relies on the webfont always loading. That is fine for production and
  // terrible for a prototype: without the licensed font the browser falls back to
  // its default *serif*, which reads nothing like FormFactor. So we append a
  // geometric-humanist sans chain. Proxima Nova still wins whenever it is present.
  const hasFallback = typo.baseFamily.includes(',');
  const fallback = 'system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif';
  L.push(`  --ff-font-sans: ${typo.baseFamily}${hasFallback ? '' : `, ${fallback}`};`);
  if (!hasFallback) {
    L.push('  /* ^ Proxima Nova is FormFactor\'s licensed typeface and is not redistributed');
    L.push('     here. The fallback chain is added by this generator, not observed. */');
  }
  L.push(`  --ff-text-base: ${typo.baseSize}px;`);
  L.push('  /* Heading sizes are responsive — see the media queries below. */');
  for (const w of tallyByNorm(typoData.weights).slice(0, 4).sort((a, b) => a - b)) {
    L.push(`  --ff-weight-${WEIGHT_NAMES[w] ?? 'w' + w}: ${w};`);
  }

  // Geometry
  L.push('');
  L.push('  /* ---- Geometry ---- */');
  L.push('  --ff-radius: 30px;        /* the one radius: cards AND buttons. 20px below 480px. */');
  L.push('  --ff-radius-field: 20px;  /* form controls only — the single deliberate exception */');
  L.push('  --ff-radius-full: 100%;   /* circular icon buttons (social share) */');
  const otherRadii = radiusUniform.filter(([v]) => !/^(30px|20px|100%)$/.test(v));
  if (otherRadii.length) {
    L.push(`  /* Also observed, all from plugin defaults rather than theme markup: ${otherRadii.map(([v, c]) => `${v} (${c}x)`).join(', ')} */`);
  }
  L.push('');
  if (shadow[0]) L.push(`  --ff-shadow: ${shadow[0][0]};`);
  if (shadow[1]) L.push(`  --ff-shadow-soft: ${shadow[1][0]};`);
  L.push('  /* There is no elevation scale. Two shadows exist; most surfaces use the first. */');

  // Layout
  L.push('');
  L.push('  /* ---- Layout ---- */');
  L.push('  --ff-wrapper-max: 1433px;  /* resolved at >=1435px. ~1040px below that. */');
  L.push('  --ff-header-height: 92px;');
  L.push('  --ff-logo-width: 340px;');
  L.push('');
  L.push('  /* ---- Breakpoints (for reference — CSS vars cannot be used in media queries) ---- */');
  for (const bp of THEME_BREAKPOINTS) {
    L.push(`  --ff-bp-${bp.name}: ${bp.minWidth}px;${' '.repeat(Math.max(1, 10 - bp.name.length))}/* ${bp.label}${bp.suffix ? `, suffix ${bp.suffix}` : ''} */`);
  }
  L.push('}');

  // Responsive overrides — mirror the theme's own behaviour.
  L.push('');
  L.push('/* Below the tablet breakpoint the pill tightens. This single override is why');
  L.push('   every card and button on the real site is responsive without a per-component');
  L.push('   media query — they all read --ff-radius through one utility. */');
  L.push('@media (max-width: 479px) {');
  L.push('  :root {');
  L.push('    --ff-radius: 20px;');
  L.push('    --ff-logo-width: 224px;');
  L.push('  }');
  L.push('}');
  L.push('');
  L.push('@media (max-width: 1434px) {');
  L.push('  :root { --ff-wrapper-max: 1040px; }');
  L.push('}');

  // Heading scale, responsive — the real authored ladder.
  L.push('');
  L.push('/* Heading scale. Three effective steps, not four: the 480px and 1024px bands');
  L.push('   share one scale and only >=1435px steps up. Sizes are the authored');
  L.push('   calc(var(--size-N) * Xrem) values, resolved. */');
  L.push(':root {');
  L.push('  --ff-text-h1: 37.8px;  --ff-text-h2: 34px;');
  L.push('  --ff-text-h3: 25.5px;  --ff-text-h4: 21.3px;');
  L.push('}');
  L.push('@media (min-width: 480px) {');
  L.push('  :root {');
  L.push('    --ff-text-h1: 50.4px;  --ff-text-h2: 37.8px;');
  L.push('    --ff-text-h3: 24.5px;  --ff-text-h4: 21.3px;');
  L.push('  }');
  L.push('}');
  L.push('@media (min-width: 1435px) {');
  L.push('  :root {');
  L.push('    --ff-text-h1: 67.2px;  --ff-text-h2: 45.3px;');
  L.push('    --ff-text-h3: 30px;    --ff-text-h4: 24.7px;');
  L.push('  }');
  L.push('}');
  L.push('');
  return L.join('\n');
...
async function main(): Promise<void> {
  ...
  // CSS custom properties (P6) — the consumable form of the tokens.
  await fs.writeFile(path.join(TOKENS_DIR, 'tokens.css'), buildTokensCss({
    clusters, scale, typo: { baseFamily, baseSize }, typoData,
    radiusUniform, shadow: shadowOrdered, respVars,
  }));
